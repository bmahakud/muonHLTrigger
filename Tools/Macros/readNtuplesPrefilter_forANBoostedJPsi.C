...
void readNtuplesPrefilter_forANBoostedJPsi(TString inputfilename="muonNtuple.root", int flavor=Sig::Prompt, std::string effmeasured="IterL3_"){

  bool doingL1 = thepassfilter.find("L1fL1") != std::string::npos; 

  TFile* outfile = TFile::Open(Form("%s_efficiencyBoostedJPsi_pre.root", effmeasured.c_str()),"RECREATE");
  std::cout << "output file: " << outfile -> GetName() << std::endl;

  //Create histograms  
  TH1F* dimuon_mass             = new TH1F("h_dimuon_mass"          ,"dimuon_mass"      , 50,  0,  15 );
  TH1F* TagProbe_pt             = new TH1F("h_TagProbe_pt"          ,"TagProbe_pt"      , 1000,  0,  500 );
  ...
  for (Int_t eventNo=0; eventNo < 10000; eventNo++)     {
    ...
    for (int imu = 0; imu < nmuons; imu++){ 
     ...
     if (! selectTagMuon(ev -> muons.at(imu), tagiso)) continue; 
      cout<<"Muon pt: "<<ev -> muons.at(imu).pt<<endl;     
 
      if (! matchMuon(ev -> muons.at(imu), ev -> hltTag.objects, isofilterTag)) continue;

      
      tagMuonPt -> Fill ( ev -> muons.at(imu).pt) ; 
      TLorentzVector TagOff;
      TagOff.SetPtEtaPhiM(ev -> muons.at(imu).pt,ev -> muons.at(imu).eta,ev -> muons.at(imu).phi,0.0); 
	...
	if (!doingL1 && !(matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, theprobefilter))) continue;	
       ...
       TLorentzVector ProbeOff;
       ProbeOff.SetPtEtaPhiM(ev -> muons.at(jmu).pt,ev -> muons.at(jmu).eta,ev -> muons.at(jmu).phi,0.0);

       TLorentzVector TP;
       TP=TagOff+ProbeOff;
       TagProbe_pt->Fill(TP.Pt());

       //if(TP.Pt() <60.0)continue;
...
bool selectProbeMuon(MuonCand mu, MuonCand tagMu, TH1F* dimuon_mass){
  ...
  if (! (mumumass > 1. && mumumass < 5. )) return false;
