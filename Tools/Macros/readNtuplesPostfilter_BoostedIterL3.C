...
float offlinePtCut         = 2.;

void readNtuplesPostfilter_BoostedIterL3(TString inputfile ="/eos/uscms/store/user/bmahakud/BoostedJPshi_LPCAutumn/JPsiToMuMu_Pt20to100-pythia8-gun/BoostedJPshi_LPCAutumn/190131_151629/0000/BoostedJPsi.root", std::string effmeasured="BoostedJPsi_"){
  ...
  TH1F *pairMass =new TH1F("h_pairMass","h_pairMass",50,0,5);
  TH1F *pairMassDY =new TH1F("h_pairMassDY","h_pairMassDY",35,50,120);

  TH1F *pairPt =new TH1F("h_pairPt","h_pairPt",250,0,500);
  TH1F *pairEta =new TH1F("h_pairEta","h_pairEta",10,-2.4,2.4);
  TH1F *openingAngle=new TH1F("h_openingAngle","h_openingAngle",300,0,10);
  ...
  for (Int_t eventNo=0; eventNo < 25000; eventNo++)     {
    Int_t IgetEvent   = tree   -> GetEvent(eventNo);
    printProgBar((int)(eventNo*100./25000));
...
//	if (!doingL1 && !(matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, theprobefilter))) continue;
        ...
        muon1Pt->Fill(ev -> muons.at(imu).pt);
        muon2Pt->Fill(ev -> muons.at(jmu).pt);
...
//	if (matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, thepassfilter) && matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, thepassfilter))  pass = true;  


//        if(matchMuonWithL3(ev -> muons.at(jmu), ev -> hlt.objects) && matchMuonWithL3(ev -> muons.at(imu), ev -> hlt.objects))pass =true;


        if(matchBothMuonsWithHLT(ev -> muons.at(imu),ev -> muons.at(jmu),ev->hltmuons))pass=true;
	...
	if (pass) { 
          openingAnglePass->Fill(opening_Angle);
	  ...
	  FailingProbeMll -> Fill( mumumass                );
        ...
        muonDelRDY ->Fill(pass, opening_Angle);
	muonDelRJsi ->Fill(pass, opening_Angle);
...
float getLeadingPtCut(int signature){ 
  float ptcut = 0.;
  if (signature == Sig::Prompt) ptcut = 2;//29.;
