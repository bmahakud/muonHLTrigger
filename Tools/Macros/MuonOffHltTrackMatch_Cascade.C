...
vector<size_t> matchHltTrackMu ( std::vector<MuonCand>, std::vector<HltTrackCand>, TH1F*, TH1F* , TH1F*);


void MuonOffHltTrackMatch_Cascade(TString inputfilename="/afs/cern.ch/user/s/sferrere/private/CMSSW_9_2_7/src/workspace/cascade/JSON_08_16/results_alignment/results.root", std::string effmeasured="CASCADE"){

  TFile* outfile = TFile::Open(Form("%sCascade_Mu_HltTrack_Match.root", effmeasured.c_str()),"RECREATE");
  ...
  TH1F* pt_muons        = new TH1F("h_pt_muon"       , "pt_muon"       ,  200,   0,  150);
  ...
  TH1F* eta_hltOI   = new TH1F("h_eta_hltOI"   , "eta_hltOI"  , 1000,  -2.5,  2.5 );
  TH1F* pt_hltOI    = new TH1F("h_pt_hltOI"    , "pt_hltOI"   ,  150,     0,  150 );
  TH1F* eta_resta   = new TH1F("h_eta_resta"   , "eta_resa"   , 1000,  -2.5,  2.5 );
  ...
  TH1F* PixHitOI_int = new TH1F("h_PixHitOI_int" , "PixHitOI_int" ,    21, -10,     10 );
  TH1F* LayHitOI_int = new TH1F("h_LayHitOI_int" , "LayHitOI_int" ,    25, -12,     12 );
  TH1F* PixLayOI_int = new TH1F("h_PixLayOI_int" , "PixLayOI_int" ,    25, -12,     12 );
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++) {
    ...
    vector< size_t> matchTrackOI   = matchHltTrackMu(ev -> muons, ev -> hltTrackOI, DRforOI, eta_hltOI, pt_hltOI);
...
vector<size_t> matchHltTrackMu (std::vector<MuonCand> mu, std::vector<HltTrackCand> hltTrack, TH1F* DeltaRplot, TH1F* eta_plot, TH1F* pt_plot){
  ...
  for ( int ihlt=0; ihlt<hltTrack.size(); ihlt++ ) {
    ...
    pt_plot -> Fill ( hltTrack.at(ihlt).pt );
  }
