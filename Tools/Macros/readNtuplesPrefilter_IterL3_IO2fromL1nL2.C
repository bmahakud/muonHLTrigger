...
void readNtuplesPrefilter_IterL3_IO2fromL1nL2(TString inputfilename="/eos/uscms/store/user/bmahakud/ProductionHLTAN_LPC_IterL3HighStat/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/ProductionHLTAN_LPC_IterL3HighStat/181130_193653/0000/muonNtupleIterL3.root", std::string effmeasured="IO2FromL1nL2"){

  bool doingL1 = thepassfilter.find("L1fL1") != std::string::npos; 

  TFile* outfile = TFile::Open(Form("%s_IO2FromL1nL2_effpreFilter.root", effmeasured.c_str()),"RECREATE");
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++) 
	...
	for (int jmu = imu+1; jmu < nmuons; jmu++){
	  ...
	  //PREFILTER
	  if (matchMuonWithL3(ev->muons.at(jmu),ev->hltTrackIOL2) || matchMuonWithL3(ev->muons.at(jmu),ev->hltTrackIOL1)) pass = true; 
