...
void readNtuplesPrefilter_IterL3_IOFromL1(TString inputfilename="/eos/uscms/store/user/bmahakud/NTuples_Jan24_2019/IOfromL1_ROI0p25p20/IOfromL1_ROI0p25p20.root", std::string effmeasured="IOFromL1_ROI0p25p20"){
 ...
 ///eos/uscms/store/user/bmahakud/ProductionHLTAN_LPC_IterL3HighStat/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/ProductionHLTAN_LPC_IterL3HighStat/181130_193653/0000/muonNtupleIterL3.root
  ...
  TFile* outfile = TFile::Open(Form("%s_EffIterL3_IOFromL1_preFilter.root", effmeasured.c_str()),"RECREATE");
  ...
  int nentries = 1000000;//tree->GetEntries();
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++) 
	...
	for (int jmu = imu+1; jmu < nmuons; jmu++){
	  ...
	  //PREFILTER
	  if (matchMuonWithL3(ev->muons.at(jmu),ev->hltTrackIOL1)) pass = true; 
