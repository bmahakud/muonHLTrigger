...
void readNtuplesPrefilter_SharedHitsIterL3(TString inputfilename="muonNtupleOntheFly.root", std::string effmeasured="MC2018"){
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++)     {
    ...
    for (int imu = 0; imu < nmuons; imu++){ 
      ...
      for(int i=0;i<ev->hltmuons.size();i++){
      cout<<"Shared Hit fraction hlt muon: "<<i<<":  "<<ev->muons.at(imu).SharedHitFrac.at(i)<<endl;
