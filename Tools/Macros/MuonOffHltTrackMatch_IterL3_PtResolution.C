...
void MuonOffHltTrackMatch_IterL3_PtResolution(TString inputfilename="/eos/uscms/store/user/bmahakud/NTuples_Jan24_2019/Standard/muonNtuple_*.root", std::string effmeasured="IterL3_ForJan"){

  TFile* outfile = TFile::Open(Form("%s_Mu_HltTrack_Match.root", effmeasured.c_str()),"RECREATE");
  std::cout << "output file: " << outfile -> GetName() << std::endl;

  TH1F *hdxy_onlineOI =new TH1F("hdxy_onlineOI","hdxy_onlineOI",2000,-10,10);
  TH1F *hdxy_onlineTk =new TH1F("hdxy_onlineTk","hdxy_onlineTk",2000,-10,10);
  TH1F *hdxy_offline =new TH1F("hdxy_offline","hdxy_offline",2000,-10,10);
  TH1F *hdxy_resol =new TH1F("hdxy_resol","hdxy_resol",2000,-1,1);
  ...
  for (Int_t eventNo=0; eventNo < 20000; eventNo++) {

    Int_t IgetEvent   = tree   -> GetEvent(eventNo);
    printProgBar((int)(eventNo*100./20000));
    ...
    //vector<size_t> matchTrackIO = matchHlt
    ...
    for (size_t imu = 0; imu < nmuons; imu++){
      ...
      float muondxy=ev -> muons.at(imu).dxy;

      //hdxy_online
      
      if(ihltOI > -1 && itkmu > -1){
       hdxy_onlineOI->Fill(ev->hltTrackOI.at(ihltOI).dxy);
      // hdxy_onlineTk->Fill(ev->tkmuons.at(itkmu).dxy);
       hdxy_offline->Fill(muondxy);
       hdxy_resol->Fill((muondxy-ev->hltTrackOI.at(ihltOI).dxy)/muondxy);
