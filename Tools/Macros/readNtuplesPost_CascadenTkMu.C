...
std::string thepassfilter2 ="hltL3fL1sMu22Or25f0TkFiltered27Q::TEST";//"hltL3fL1sMu22Or25L1f0Tkf27QL3trkIsoFiltered0p07::TEST";////"hltL3fL1sMu22Or25f0TkFiltered27Q::TEST";
std::string thepassfilter  = "hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p07::TEST";//"hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p07::TEST";//   "hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p07::TEST";//L3filter;
...
void readNtuplesPost_CascadenTkMu(TString inputfilename="/eos/uscms/store/user/bmahakud/ProductionCasTest_v1/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/ProductionCasTest_v1/181205_133338/0000/CascadeTkMuCorrNew.root", std::string effmeasured="CascadeTkMu_New_CorrectedTrigger"){
...
//latest /eos/uscms/store/user/bmahakud/ProductionCasTest_v1/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/ProductionCasTest_v1/181205_133338/0000/CascadeTkMuCorrNew.root


  bool doingL1 = thepassfilter.find("L1fL1") != std::string::npos; 

  TFile* outfile = TFile::Open(Form("%s_CascadenTkMu_postfilter.root", effmeasured.c_str()),"RECREATE");
  ...
  TEfficiency* muonchi2_barrel       = new TEfficiency("muonchi2_barrel"      , "muonchi2_barrel"      ,  60,    0.,   7.);
  ...
  TEfficiency* muonchi2_int     = new TEfficiency("muonchi2_int"     , "muonchi2_int"      ,  60,   0.,  15.);
  ...
  TEfficiency* muonchi2_endcap     = new TEfficiency("muonchi2_endcap"     , "muonchi2_endcap"      ,  60,   0.,   7.);
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++) 
	...
	for (int jmu = imu+1; jmu < nmuons; jmu++){
	  ...
	  if (!doingL1 && !(matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, theprobefilter))) continue;
          bool passCascade=false;
          bool passTkMu=false;

          if(matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, thepassfilter))passCascade=true;
          if(matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, thepassfilter2))passTkMu=true;
	 // if (matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, thepassfilter) || matchMuon(ev -> muons.at(jmu), ev -> hlt.objects, thepassfilter2))  pass = true;
          if( passCascade || passTkMu ) pass =true;
