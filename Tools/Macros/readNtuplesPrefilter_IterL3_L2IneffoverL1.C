...
void readNtuplesPrefilter_IterL3_L2IneffoverL1(TString inputfilename="/eos/uscms/store/user/bmahakud/ProductionHLTAN_LPC_IterL3HighStat/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/ProductionHLTAN_LPC_IterL3HighStat/181130_193653/0000/muonNtupleIterL3.root", std::string effmeasured="IterL3_L2EffwrtL1_"){
  ...
  TFile* outfile = TFile::Open(Form("%s_IterL3_L2inefficiencyOverL1_preFilter.root", effmeasured.c_str()),"RECREATE");
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++)     {
	...
	if (matchMuonWithL3(ev->muons.at(jmu),ev->L2muons)) pass = false;
