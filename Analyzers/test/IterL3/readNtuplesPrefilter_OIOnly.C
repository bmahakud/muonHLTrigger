...
void readNtuplesPrefilter_OIOnly(TString inputfilename="/eos/uscms/store/user/bmahakud/ProductionHLTAN_LPC/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/ProductionHLTAN_LPC/181130_154917/0000/muonNtupleData.root", std::string effmeasured="Navigation"){
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++) 
	...
	//cout << "select tag" << endl; 
        //cout<<"isofilter: "<<isofilterTag<<endl;
  ...
  cout <<"total muons: "<<total<<endl;
  cout <<"total pass muons: "<<totalPass<<endl; 
...
bool matchMuon(MuonCand mu, std::vector<HLTObjCand> toc, std::string tagFilterName){
  ...
  for ( std::vector<HLTObjCand>::const_iterator it = toc.begin(); it != toc.end(); ++it ) { 
    //cout<<"given: "<<tagFilterName<<endl;
    //cout<<"exist: "<<it->filterTag<<endl;
